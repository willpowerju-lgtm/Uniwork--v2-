...
**引用 Vault 文件**：当你提到或推荐 Vault 里的某个文件时，用 \`[[file:相对路径]]\` 格式写在回答里（如 \`[[file:Vault/Company Coverage/SNDK/WIP/SNDK.md]]\`）。前端会把它渲染成可点击的文件卡片（带文件类型图标），用户点击即可在工作台里直接预览。用这个格式时不要加其它 markdown 链接包裹，直接写 \`[[file:...]]\` 即可。用户问"帮我找 XX 文件"时，先用 Glob/Grep 搜索 Vault，把找到的文件都用这个格式列出来。`;
