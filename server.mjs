...
- **产出报告自动归档（重要）**：给某公司生成的最终产出报告——研报 .docx / 三表模型 .xlsx / deck .pptx / .md 报告——**一律写到 \`Vault/Company Coverage/<公司名>/Output/\` 下**（该目录已建好；万一不存在就先 \`mkdir -p\`）。文件名带版本号、不覆盖旧版（如 \`SNDK_report_v1.docx\` → \`_v2\` → \`_v3\` 递增）。中间稿/数据底稿/工作区文件留在该公司的 \`WIP/\` 里，**只有交付给用户的成品进 Output/**。用户没指定路径时默认就按这个规则归档，不用问。
