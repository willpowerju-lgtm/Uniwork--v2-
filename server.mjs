...
wss.on('connection', (ws) => {
  ...
  ws.on('close', () => {
    clients.delete(conn);
    for (const tab of conn.tabs.values()) { try { tab.channel.close(); } catch {} try { tab.query?.interrupt?.(); } catch {} }
    conn.tabs.clear();
  });
});
...
async function handleMessage(conn, raw) {
  ...
  switch (m.op) {
    ...
    case 'interrupt': { const tab = conn.tabs.get(m.tabId); if (tab?.query?.interrupt) { try { await tab.query.interrupt(); } catch {} send(ws, { ev: 'interrupted', tabId: m.tabId }); } break; }
...
async function userTurn(conn, m) {
  ...
  // 真 SDK：首条消息时建会话
  if (!tab) {
    tab = await startTab(conn, tabId, m.model || DEFAULT_MODEL, m.openFile);
...
async function startTab(conn, tabId, model, openFile) {
  const channel = new MessageChannel();
  const abort = new AbortController();
  const tab = { channel, abort, query: null, sessionId: null, model, openFile, consumer: null };
...
| python-docx | Bash 跑 python + python-docx | 读写 .docx（改段落/表格/样式）|
